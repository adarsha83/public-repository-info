                ...
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        if (issuesUrl != '') {
                            ...
                            url = issuesUrl.replace('{/number}', '') + '?accessToken=' + accessToken;
                            $.ajax({
                                url: url,
                                type: 'GET',
                                dataType: "json",
                                success: function (issues) {
                                    ...
                                    for (var i = 0; i < issues.length; i++) {
                                        ...
                                        if (isValidCreatedAt(issues[i])) {
                                            if (getDateDifferenceInHours(today, new Date(issues[i].created_at)) <= 24) {
                                                count2++;
                                            }
                                            if (getDateDifferenceInHours(today, new Date(issues[i].created_at)) >= 24 && getDateDifferenceInDays(today, new Date(issues[i].created_at)) <= 7) {
                                                count3++;
                                            }
                                            if (getDateDifferenceInDays(today, new Date(issues[i].created_at)) > 7) {
                                                count4++;
                                            }
                                        }
                                    }
